...
var $author$project$Block$data = F2(
	function (id, quantity) {
		return {headerOffset: 2, id: id, quantity: quantity, width: 10, x: 0, y: 0};
	});
var $author$project$Grid$emptyParams = {alternateCount: 0, height: 0, offset: 0, width: 0, x: 0, y: 0};
...
var $author$project$Main$init = function (_v0) {
	var m = {
		blocks: _List_fromArray(
			[
				A2(
				$author$project$Block$withPos,
				_Utils_Tuple2(5, 5),
				A2($author$project$Block$data, '1', 36)),
				A2(
				$author$project$Block$withPos,
				_Utils_Tuple2(20, 5),
				A2($author$project$Block$data, '2', 54))
			]),
...
var $author$project$Grid$centeredParams = F2(
	function (wh, offset) {
		var svgWidth = wh.a;
		var svgHeight = wh.b;
		var gridWidth = ((svgWidth / offset) | 0) * offset;
		var x = ((svgWidth - gridWidth) / 2) | 0;
		var gridHeight = ((svgHeight / offset) | 0) * offset;
		var y = ((svgHeight - gridHeight) / 2) | 0;
		return {alternateCount: 0, height: gridHeight, offset: offset, width: gridWidth, x: x, y: y};
	});
var $elm$core$Basics$min = F2(
	function (x, y) {
		return (_Utils_cmp(x, y) < 0) ? x : y;
	});
var $author$project$Grid$getOffset = F2(
		...
		switch (msg.$) {
			...
			case 'SizeChanged':
				...
				var g = A2(
					...
					wh,
					A2($author$project$Grid$getOffset, wh, 15));
				var g_ = _Utils_update(
					g,
					{alternateCount: 5});
				return _Utils_Tuple2(
					_Utils_update(
...
var $author$project$Grid$lineClass = F3(
	function (index, alternateCount, baseClass) {
		return ((!(!index)) && (!A2($elm$core$Basics$modBy, alternateCount, index))) ? (baseClass + '-alternate') : baseClass;
	});
var $author$project$Grid$hline = F2(
	function (p, index) {
		var offset = index * p.offset;
		var p1 = _Utils_Tuple2(p.x, p.y + offset);
		var p2 = _Utils_Tuple2(p.x + p.width, p.y + offset);
		var _class = A3($author$project$Grid$lineClass, index, p.alternateCount, 'hline');
...
var $author$project$Grid$vline = F2(
	function (p, index) {
		...
		var _class = A3($author$project$Grid$lineClass, index, p.alternateCount, 'vline');
		return A3($author$project$Grid$line, p1, p2, _class);
	});
var $author$project$Grid$view = function (params) {
	var verticalLines = A2(
		...
		A2($elm$core$List$range, 0, (params.width / params.offset) | 0));
	var horizontalLines = A2(
		$elm$core$List$map,
		$author$project$Grid$hline(params),
		A2($elm$core$List$range, 0, (params.height / params.offset) | 0));
	return _Utils_ap(horizontalLines, verticalLines);
};
...
var $author$project$Block$rect = function (vd) {
	return _List_fromArray(
		[
			A2(
			$elm$svg$Svg$rect,
			_List_fromArray(
				[
					...
					$elm$core$String$fromInt(vd.x)),
					$elm$svg$Svg$Attributes$y(
					$elm$core$String$fromInt(vd.y)),
					$elm$svg$Svg$Attributes$width(
					$elm$core$String$fromInt(vd.width)),
					$elm$svg$Svg$Attributes$height(
					$elm$core$String$fromInt(vd.height)),
...
var $author$project$Block$toLogicalViewData = function (bd) {
	...
	var header = {_class: 'block-header', height: 1, width: headerWidth, x: bd.x + bd.headerOffset, y: bd.y};
...
var $author$project$Block$toPhysicalViewData = F2(
	function (gd, vd) {
		return {_class: vd._class, height: vd.height * gd.offset, width: vd.width * gd.offset, x: gd.x + (gd.offset * vd.x), y: gd.y + (vd.y * gd.offset)};
	});
var $author$project$Block$view = F2(
	function (gd, bd) {
		var parts = $elm$core$List$concat(
			A2(
				$elm$core$List$map,
				$author$project$Block$rect,
				A2(
					$elm$core$List$map,
...
var $author$project$Block$viewAll = F2(
	function (gd, bds) {
		return A2(
			$elm$core$List$map,
			$author$project$Block$view(gd),
			bds);
	});
var $author$project$Main$view = function (m) {
	var parts = _List_fromArray(
		[
			...
			A2($author$project$Block$viewAll, m.grid, m.blocks)
		]);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
