...
var $author$project$Block$clearSelection = function (_v0) {
	var bd = _v0.a;
...
var $author$project$Block$Group$clearSelection = function (group) {
	var active_ = $elm_community$maybe_extra$Maybe$Extra$toList(
		A2($elm$core$Maybe$map, $author$project$Block$clearSelection, group.active));
	return _Utils_update(
		group,
		{
			active: $elm$core$Maybe$Nothing,
...
var $author$project$Pos$deltaBetween = F2(
	function (start, end) {
		return A2($author$project$Delta$Delta, end.x - start.x, end.y - start.y);
	});
		...
		switch (msg.$) {
			...
			case 'AddBlock':
				...
				return _Utils_Tuple2(
					_Utils_update(
						m,
						{blocks: blocks_, key: key_}),
					$elm$core$Platform$Cmd$none);
			case 'ClearSelection':
				var blocks_ = $author$project$Block$Group$clearSelection(m.blocks);
				return _Utils_Tuple2(
					_Utils_update(
						m,
						{blocks: blocks_}),
...
var $author$project$Main$AddBlock = {$: 'AddBlock'};
		...
		var _v0 = function () {
			...
			if ((_v1.$ === 'Dragging') && (_v1.a.$ === 'Offset')) {
				...
				return {
					...
					guideVisible: true,
					radius: $author$project$Block$Internal$Config$circleActiveSize(vm.grid.unit),
					rootpos: A2($author$project$Pos$addDelta, drag.delta.current, drag.start)
				};
			} else {
				var root = $author$project$Block$Internal$Component$Offset$rootPosition(vm);
				return {
					cpos: A2($author$project$Block$Internal$Component$Offset$circlePosition, vm, root),
					guideVisible: false,
					radius: $author$project$Block$Internal$Config$circleIdleSize(vm.grid.unit),
					rootpos: root
				};
			}
		}();
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$svg$Svg$Attributes$cx(
							$author$project$Pos$toXString(btnPos)),
							$elm$svg$Svg$Attributes$cy(
							$author$project$Pos$toYString(btnPos)),
							$elm$svg$Svg$Attributes$r(
